...
abstract class BaseService<T> {
  ...
  Future<List<T>?> getAllByIdBase2(
      String endpoint, endpoint2, id, Map<String, String> request) async {
    SharedStates sharedStates = Get.find();
    String token = sharedStates.token;
    Response response = await _apiHelper.getById1(endpoint, endpoint2, id,
  ...
  Future<bool> putBase2(String endpoint, id, Map<String, String> request,
      Map<String, dynamic>? body) async {
    SharedStates sharedStates = Get.find();
    String token = sharedStates.token;
    Response res = await _apiHelper.putOne1(endpoint, id, body,
        request: {'Authorization': 'bearer $token'});
    print(res.statusCode.toString());
    // print("test" + body.toString());
    if (res.statusCode == HttpStatus.ok) {
